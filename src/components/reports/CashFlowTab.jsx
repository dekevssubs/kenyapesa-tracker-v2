...
export default function CashFlowTab({ dateRange }) {
  ...
  const fetchCashFlowData = async () => {
    try {
      ...
      const allTransactions = [
        ...(incomeData.data?.map(t => ({ ...t, type: 'income', amount: parseFloat(t.amount) })) || []),
        ...(expenseData.data?.map(t => ({ ...t, type: 'expense', amount: parseFloat(t.amount) })) || [])
      ...
      const flowData = allTransactions.map(transaction => {
        ...
        return {
          ...
          flowAmount,
          cumulativeFlow,
          description: transaction.description || transaction.source || transaction.category
        }
      })
      ...
      const largestInflow = incomeData.data
        ?.map(t => ({ ...t, amount: parseFloat(t.amount) }))
        .sort((a, b) => b.amount - a.amount)[0]

      const largestOutflow = expenseData.data
        ?.map(t => ({ ...t, amount: parseFloat(t.amount) }))
        .sort((a, b) => b.amount - a.amount)[0]
  ...
  const getHealthBgColor = (health) => {
    switch (health) {
      case 'excellent': return 'from-green-50 to-green-100 border-green-200'
      case 'good': return 'from-blue-50 to-blue-100 border-blue-200'
      case 'fair': return 'from-yellow-50 to-yellow-100 border-yellow-200'
      case 'poor': return 'from-red-50 to-red-100 border-red-200'
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="card bg-gradient-to-br from-green-50 to-green-100 border-green-200">
          ...
          <p className="text-3xl font-bold text-green-900">{formatCurrency(insights.totalInflow)}</p>
        ...
        <div className="card bg-gradient-to-br from-red-50 to-red-100 border-red-200">
          ...
          <p className="text-3xl font-bold text-red-900">{formatCurrency(insights.totalOutflow)}</p>
        ...
        <div className="card bg-gradient-to-br from-purple-50 to-purple-100 border-purple-200">
          <div className="flex items-center mb-2">
            ...
            <p className="text-sm text-purple-700 font-medium">Cash Flow Days</p>
          </div>
          <div className="flex items-center space-x-4 mt-2">
            <div>
              <p className="text-2xl font-bold text-green-700">{insights.positiveFlowDays}</p>
              <p className="text-xs text-gray-600">Positive</p>
            </div>
            <div className="border-l border-purple-300 h-12"></div>
            <div>
              <p className="text-2xl font-bold text-red-700">{insights.negativeFlowDays}</p>
      ...
      {weeklyData.length > 1 && (
        <div className="card">
          <h3 className="text-lg font-bold text-gray-900 mb-4">Weekly Cash Flow Summary</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="card bg-gradient-to-br from-green-50 to-emerald-50 border-green-200">
          <div className="flex items-center mb-4">
            ...
            <h3 className="text-lg font-bold text-gray-900">Largest Inflow</h3>
          </div>
          {insights.largestInflow ? (
            <div className="space-y-2">
              <p className="text-3xl font-bold text-green-900">{formatCurrency(insights.largestInflow.amount)}</p>
              <div className="text-sm">
                <p className="text-gray-700">
                  <span className="font-semibold">Source:</span> {insights.largestInflow.source || 'N/A'}
                </p>
                <p className="text-gray-700">
            ...
            <p className="text-gray-600">No inflow data</p>
          )}
        </div>
        ...
        <div className="card bg-gradient-to-br from-red-50 to-rose-50 border-red-200">
          <div className="flex items-center mb-4">
            ...
            <h3 className="text-lg font-bold text-gray-900">Largest Outflow</h3>
          </div>
          {insights.largestOutflow ? (
            <div className="space-y-2">
              <p className="text-3xl font-bold text-red-900">{formatCurrency(insights.largestOutflow.amount)}</p>
              <div className="text-sm">
                <p className="text-gray-700">
            ...
            <p className="text-gray-600">No outflow data</p>
          )}
        </div>
      </div>
      ...
      <div className={`card bg-gradient-to-r ${getHealthBgColor(insights.cashFlowHealth)}`}>
        <h3 className="text-lg font-bold text-gray-900 mb-4">Cash Flow Health Assessment</h3>
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4 pt-4 border-t">
            <div>
              <p className="text-xs text-gray-600 mb-1">Inflow/Outflow Ratio</p>
            ...
            <div>
              <p className="text-xs text-gray-600 mb-1">Positive Days</p>
              <p className="text-xl font-bold text-green-700">
                {((insights.positiveFlowDays / dailyData.length) * 100).toFixed(0)}%
              </p>
            </div>
            <div>
              <p className="text-xs text-gray-600 mb-1">Negative Days</p>
